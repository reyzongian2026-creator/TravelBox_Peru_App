...
function Write-Step {
    ...
    Write-Progress -Id 3 -Activity "TravelBox production deploy (ENV-only)" -Status $Status -PercentComplete $Percent
...
function Invoke-GcloudWithFrontendUploadProgress {
    param(
        [string]$GcloudPath,
        [string[]]$Arguments,
        [int]$MainProgressId = 3,
        [int]$UploadProgressId = 33,
...
function Get-EnvVarRequired {
    param([string]$Name)
    $raw = [Environment]::GetEnvironmentVariable($Name)
    if ($null -eq $raw) {
        throw ("Falta variable de entorno requerida: {0}" -f $Name)
    }
    $value = $raw.Trim()
    if ([string]::IsNullOrWhiteSpace($value)) {
        throw ("Variable de entorno vacia: {0}" -f $Name)
    }
    return $value
}

function Get-EnvVarOptional {
    param([string]$Name)
    $raw = [Environment]::GetEnvironmentVariable($Name)
    if ($null -eq $raw) {
        return ""
...
if (-not (Test-Path $backendProdEnvFile)) {
    throw "No existe env file prod: $backendProdEnvFile"
}

Write-Step -Percent 5 -Status "Leyendo variables de entorno requeridas..."
$azureClientId = Get-EnvVarRequired -Name "AZURE_CLIENT_ID"
$azureClientSecret = Get-EnvVarRequired -Name "AZURE_CLIENT_SECRET"
$azureTenantId = Get-EnvVarRequired -Name "AZURE_TENANT_ID"
...
if (-not $SkipBackendDeploy) {
    Write-Step -Percent 45 -Status "Desplegando backend prod en Cloud Run (ENV-only)..."
    $backendEnvFileForDeploy = $backendProdEnvFile
    $tempBackendEnvFile = $null
    try {
        $tempBackendEnvFile = Join-Path $appRoot ("tmp.cloudrun-backend-env.prod.envonly.{0}.yaml" -f ([Guid]::NewGuid().ToString("N")))
...
$cashOnlyFlag = Get-EnvVarOptional -Name "APP_PAYMENTS_FORCE_CASH_ONLY"
...
foreach ($defineName in $firebaseDefineNames) {
    $defineValue = Get-EnvVarOptional -Name $defineName
...
if (-not $SkipFrontendDeploy) {
    ...
    if ($NoExecute) {
        ...
        Invoke-GcloudWithFrontendUploadProgress -GcloudPath $gcloud -Arguments $frontendDeployArgs -MainProgressId 3 -UploadProgressId 33
    }
} else {
...
Write-Progress -Id 3 -Activity "TravelBox production deploy (ENV-only)" -Completed

Write-Host ""
Write-Host "Resumen despliegue produccion (ENV-only)"
